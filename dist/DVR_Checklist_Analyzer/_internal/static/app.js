...
async function loadChecklistStructure() {
    try {
        const res = await fetch('/api/checklist_structure');
        const data = await res.json();
        if (data.success) {
...
async function reMatchKeys(keys) {
    try {
        const res = await fetch('/api/match_keys', {
            method: 'POST',
...
modalSaveBtn.addEventListener('click', async () => {
    ...
    if (isPermanentDbAdd) {
        ...
        try {
            const res = await fetch('/api/add_database_risk', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function loadDatabaseRisks() {
    try {
        const res = await fetch('/api/database_risks');
        const data = await res.json();
        if (data.success) {
